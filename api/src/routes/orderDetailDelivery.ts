...
/**
 * @swagger
 * /api/order-detail-deliveries/country/{countryCode}:
 *   get:
 *     summary: Get order detail deliveries by country code
 *     tags: [Order Detail Deliveries]
 *     parameters:
 *       - in: path
 *         name: countryCode
 ...
 *     responses:
 *       200:
 *         description: List of order detail deliveries for the specified country
 *         content:
 *           application/json:
 *             schema:
 *               type: array
...
router.get('/country/:countryCode', (req, res) => {
  ...
  const ordersByCountry = orderDetailDeliveries.filter(odd => odd.countryCode === countryCode);
  res.json(ordersByCountry);
});
